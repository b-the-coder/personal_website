...
test.describe("single annotation creation and edit and delete workflow", () => {
  test("create, edit, and delete an annotation", async ({ page }) => {
    let newKey;

    await test.step("create annotation from selected text", async () => {
      ...
      const offererBox = await page
        .locator(".annotation-offerer")
        .boundingBox();
      ...
      const newKeys = afterPostKeys.filter(
        (key) => !beforePostKeys.includes(key)
      );
      newKey = newKeys[0];

      expect(annotationList[newKey].annotatedText).toBe(
      ...
      //verify text content with just added annotation is highlighted and has desgined attribute attached
      const textContentWithNewAnnotationAdded = page.locator(
        `[data-annotation-ids~="${newKey}"]`
      );
      await expect(textContentWithNewAnnotationAdded).toHaveCount(1);
      await expect(textContentWithNewAnnotationAdded).toHaveCSS(
        "background-color",
        "rgb(255, 249, 196)"
      );
    });

    await test.step("edit existing annotation", async () => {
      const clickedHighlight = page.locator(
        `[data-annotation-ids~="${newKey}"]`
      );
      await clickedHighlight.click();

      //get attribute list  that the clicked element
      const attributes = await clickedHighlight.evaluate((el) => {
      ...
      // TODO: fill in edit assertions once edit UI flow is finalized
    });

    await test.step("delete existing annotation", async () => {
      // TODO: fill in delete flow and assertions
    });
  });
});
